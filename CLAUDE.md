...
# Wipe EVERY session's history (all windows) but KEEP memories, then exit
./src/doit --reset
...
Sessions are isolated per shell. Each session has a folder `<repo>/.doit/history_<pid>/` (install-relative, not cwd-relative) holding `doit.jsonl` (doit's own turns), `cmdlog.tsv` (the user's terminal commands + exit status, written by the shell recorder hook), and `session.json` (the registry entry: pid, cwd, timestamps). The session `<pid>` is `DOIT_PPID`, which `doit-init.sh` **pins to the interactive shell's `$$`** and exports; `history_manager.get_history_file_path()` derives the folder from that PID (`get_session_dir`), NOT from `$PPID` — because the launcher's own `$PPID` is **not** always the interactive shell (notably the VS Code integrated terminal), so the shell must pin the id. Falls back to `os.getppid()` only without shell integration. The folder is always PID-named so sessions never collide. Each turn has an incrementing `id` and a `relevant_ids` list forming the dependency graph used by reference resolution.

**Output awareness:** doit answers questions about a previous command's output. For its own commands the output is in history; for commands the *user* ran (only command + exit status are stored, never output), Rule 11 directs the model to **re-run/pipe** the command to obtain the data when it succeeded and is safe to repeat. Rule 10 defines attribution defaults (you/we → doit's last action; I → user's last; bare reference → most recent). **Safety net:** because the LLM reference resolver can fail to link the right turn (e.g. amid many output-less manual `ls` commands), `run_single` deterministically adds `get_latest_output_turn_id()` (the most recent doit turn with real output) to the replayed turns for any follow-up (`instruction_has_context_indicator`), so "why did that fail?" / "which of these is safe to delete?" always have the previous output.

**Multi-step command plans (Rule 13):** for a task needing several commands in sequence, the tool-calling agent calls an `execute_plan` tool (`PLAN_TOOL_DEF`) with an ordered list of `{command, explanation}` steps. `_run_plan` prints the whole plan, asks ONE `[y/N]` (plan-level consent), then runs each step via `execute_bash` (regex blacklist still applies) and **stops at the first failure** (`_plan_step_failed`) so a broken step doesn't cascade; the transcript is the turn's output. Single commands still use `execute_bash_command`. Non-tool-calling mode supports plans too: the fallback JSON carries an optional `steps` array (`rule_triggered: 13`) which `run_single`'s fallback branch routes through the same `_run_plan`. `run_single` is unchanged (the plan is produced in its existing single generation call, then executed deterministically — no multi-round model loop). `_run_plan` also threads shell state across steps: a standalone `cd` (via `resolve_cd_target`) is validated with `os.path.isdir` and tracked (never subprocessed; missing dir → failed step), session-state builtins are accumulated, normal steps run with `execute_bash(cmd, cwd=…, prelude=…)`, and on success the net cwd/shell-state are hoisted to the parent shell via `_hoist_cd`/`_hoist_shell`. Tool *selection* is intent-based (Rule 13 + contrasting `FEWSHOT_TOOLCALL` examples), not a deterministic gate: `execute_plan` is for when the USER asks for several distinct actions; chaining/piping to answer a question (Rule 7/11, e.g. "which of these…") stays a single `execute_bash_command`.

**Multi-window / cross-session (Rule 12):** windows are isolated by default (each reads only its own session). The registry (`session.json` per folder, `history_manager.list_sessions`) makes other windows discoverable by PID + cwd + recency. An explicit cross-window reference (gated by `is_cross_session_reference`) is resolved to a target session — exact by PID (`extract_session_pid`), else fuzzy via `resolve_cross_session` (content/recency) with a clarify menu — and that session's relevant turns are injected, tagged, so the agent applies the task **re-grounded in the current cwd**. `is_session_list_query` answers "list the shell numbers" deterministically. A cross-session ACTIVITY query ("what did I do in the other window") is detected inside the activity-query route (which checks `is_cross_session_reference` first) and reports the target session's recent activity (`_answer_cross_session_activity`) instead of the current one. The default single-session path is untouched.
